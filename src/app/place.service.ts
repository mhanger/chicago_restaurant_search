...
export class PlaceService{
 ...
 private baseUrl: string = 'https://data.cityofchicago.org/resource/cwig-ma7x.json?';
 
 constructor(private http : Http){
  }

  getAll(searchTerm, page): Observable<Place[]>{  
    const offset = (page - 1) * 5;
    let searchStringVal = `$limit=5&$offset=${offset}&$order=inspection_id%20DESC&$where=dba_name%20like%20%27%25${searchTerm.toUpperCase()}%25%27`;
